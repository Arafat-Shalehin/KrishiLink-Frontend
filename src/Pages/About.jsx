...
const About = () => {
  return (
    <div className="bg-linear-to-b from-(--color-bg) to-(--color-surface) min-h-screen">
      {/* Hero Section */}
      <motion.section
      ...
      >
        <h1 className="mt-15 text-3xl sm:text-4xl md:text-5xl font-bold text-(--color-text) mb-4">
          About <span className="text-(--color-primary)">KrishiLink</span>
        </h1>
        <p className="text-(--color-muted) text-base sm:text-lg md:text-xl max-w-3xl mx-auto">
      ...
      >
        ...
        />
        <div>
          <h2 className="text-2xl sm:text-3xl font-semibold text-(--color-text) mb-4">
            Our Mission
          </h2>
          <p className="text-(--color-text)/90 leading-relaxed text-sm sm:text-base">
      ...
      <motion.section
        className="bg-(--color-bg) py-16 mt-10"
        initial={{ opacity: 0 }}
        whileInView={{ opacity: 1 }}
        viewport={{ once: true }}
        transition={{ duration: 0.8 }}
      >
        <div className="max-w-6xl mx-auto px-6 text-center">
          <h2 className="text-2xl sm:text-3xl font-semibold text-(--color-text) mb-10">
            What Makes Us Different
          </h2>

          <div className="grid md:grid-cols-3 gap-8">
            <div className="bg-(--color-surface) border border-(--color-border) rounded-2xl shadow-md p-6 hover:shadow-lg transition">
              <Leaf className="mx-auto text-(--color-primary) w-12 h-12 mb-4" />
              <h3 className="text-lg sm:text-xl font-semibold text-(--color-text) mb-2">
                Easy Crop Listing
              </h3>
              <p className="text-(--color-muted) text-sm sm:text-base">
            ...
            <div className="bg-(--color-surface) border border-(--color-border) rounded-2xl shadow-md p-6 hover:shadow-lg transition">
              <Users className="mx-auto text-(--color-primary) w-12 h-12 mb-4" />
              <h3 className="text-lg sm:text-xl font-semibold text-(--color-text) mb-2">
            ...
            <div className="bg-(--color-surface) border border-(--color-border) rounded-2xl shadow-md p-6 hover:shadow-lg transition">
              <Globe className="mx-auto text-(--color-primary) w-12 h-12 mb-4" />
              <h3 className="text-lg sm:text-xl font-semibold text-(--color-text) mb-2">
                Connected Community
              </h3>
              <p className="text-(--color-muted) text-sm sm:text-base">
      ...
      >
        <div>
          ...
          <p className="text-(--color-text)/90 leading-relaxed mb-4 text-sm sm:text-base">
      ...
      {/* Team / Footer */}
      <motion.section
        className="bg-(--color-primary) text-white py-16 text-center"
        initial={{ opacity: 0 }}
        whileInView={{ opacity: 1 }}
        viewport={{ once: true }}
        transition={{ duration: 0.8 }}
      >
        <Sprout className="mx-auto w-14 h-14 mb-4 text-(--color-accent)" />
