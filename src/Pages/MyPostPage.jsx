  ...
  return (
    <section className="min-h-screen text-(--color-text)">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        <h2 className="mt-10 text-2xl sm:text-3xl font-bold mb-8 text-center text-(--color-text)">
          My <span className="text-(--color-primary)">Posts</span>
        </h2>
            ...
            <tbody>
              {myCrops.length === 0 ? (
                <tr>
                  <td
                    colSpan="7"
                    className="py-10 text-center text-base text-(--color-muted)"
                  >
                    No posts found.
                  </td>
                </tr>
                  ...
                  >
                    <td className="py-2 px-4">
                      <img
                        src={crop.image}
                        alt={crop.name}
                        className="w-16 h-16 object-cover rounded-xl border border-(--color-border)"
                      />
                    </td>

                    <td className="py-2 px-4 font-semibold text-(--color-text)">
                      {crop.name}
                    </td>

                    <td className="py-2 px-4 text-(--color-muted)">
                      {crop.type}
                    </td>

                    <td className="py-2 px-4 text-(--color-text)/90">
                    ...
                    <td className="py-2 px-4">
                      <div className="flex items-center justify-center gap-3">
                        <button
                          ...
                          className="p-2 rounded-full border border-(--color-border) bg-(--color-surface) text-(--color-primary) hover:bg-[color-mix(in_srgb,var(--color-primary)_10%,transparent) hover:scale-105 transition"
          ...
          {showModal && (
            ...
            >
              <motion.div
                ...
                className="bg-(--color-surface) border border-(--color-border) rounded-2xl p-6 sm:p-8 w-full max-w-lg shadow-2xl"
              >
                <h3 className="text-xl sm:text-2xl font-bold mb-4 text-center text-(--color-text)">
                  Edit <span className="text-(--color-primary)">Crop</span>
                </h3>
                ...
                <form onSubmit={handleSave} className="space-y-3">
                  <input
                    ...
                    className="w-full rounded-xl border border-(--color-border) bg-(--color-surface) px-4 py-2.5 text-sm sm:text-base text-(--color-text) focus:outline-none focus:ring-4 focus:ring-(--color-primary)/20"
                    required
                  ...
                  <div className="flex flex-col sm:flex-row justify-end gap-3 mt-4">
                    <button
                      ...
                      className="px-4 py-2.5 rounded-xl border border-(--color-secondary) text-(--color-secondary) hover:bg-[color-mix(in_srgb,var(--color-secondary)_10%,transparent) transition"
                    >
                      Cancel
                    </button>
                    <button
                      type="submit"
                      className="px-4 py-2.5 rounded-xl bg-(--color-primary) text-white font-semibold hover:brightness-95 transition"
                    >
                      Save
                    </button>
                  </div>
